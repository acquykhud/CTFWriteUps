...
## Problem
The author give us a `.class` file, which is generated by the Java compiller.
So we find some tools that help us decompile this file to the source code.
After sometime, I found [this tool](http://www.javadecompilers.com/).

## Static analysis
This is the file we have after decompiling the `.class` file:
```java
...
```
# Solution
The code is simple, it first requires us to input a password, then the password is hashed, and it compares the hash with the hardcoded one.
We will need to use some online hashes database services to crack the hashes.
```
(env) osboxes@osboxes:~/Desktop$ nc challenges.auctf.com 30001
